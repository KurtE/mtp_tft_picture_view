...
// This is the SPI version.
//#include <RA8876_t3.h>

#if defined(_RA8876_T3_P) && !defined(_RA8876_T3)
#define _RA8876_T3
#endif

// The FlexIO versoin
//#define TEST_RA8876_P
...
//-----------------------------------------------------------------------------
    ...
    && !defined(_RA8875MC_H_) && !defined(_RA8876_T3) && !defined(_RA8876_COMMON)

// version that ships with Teensyduino
#warning using ILI9341_t3.h
...
// ST7735/ST7789
//-----------------------------------------------------------------------------
#if defined(__ST7735_t3_H_) || defined(__ST7789_t3_H_) || defined(__ST7796_t3_H_)
...
#define TFT_DC 55
#define TFT_BL 29
RA8876_t41_p tft = RA8876_t41_p(TFT_DC, TFT_CS, TFT_RST);
#pragma message "Display: RA8876_t41_p"
#endif
...
// mandatory to begin the MTP session.
    ...
    Serial.printf("CS:%u DC:%u RST:%u\n", TFT_CS, TFT_DC, TFT_RST);
    tft.begin();
    tft.setRotation(1);

#elif defined(__ST7735_t3_H) || defined(__ST7789_t3_H_) || defined(__ST7796_t3_H_)
...
#elif defined(_RA8876_T3) || defined(_RA8876_T41_P)
#if defined(TFT_BL)
    pinMode(TFT_BL, OUTPUT);
    digitalWrite(TFT_BL, HIGH);
#endif
    ...
    #endif
    tft.begin(20);
    Serial.printf("RA8876_P CS:%u DC:%u, RST:%u\n", TFT_CS, TFT_DC, TFT_RST);
#else
    tft.begin(2000000);
#endif

#if defined(RA8876_INT) && defined(USE_FT5206_TOUCH)
...
#error - did not specify init for display
    ...
    Serial.println("Before tft.println"); Serial.flush();
    tft.println(F("Init SD card..."));
    Serial.println("After print init sd card"); Serial.flush();
...
#endif
    Serial.println("Before SD.begin()"); Serial.flush();
    if (!SD.begin(SD_CS)) {
        tft.setTextSize(2);
        FillScreen(RED);
...
#if defined(_ILI9341_t3NH_) || defined(_ILI9488_t3H_) || defined(__ST7735_t3_H_) || defined(__ST7789_t3_H_) || defined(__ST7796_t3_H_)
    ...
    if (tft.getCurrentLayer() != 255) Serial.println("RA8875 - Using layers");
        ...
        Serial.println("RA8875 - Failed to use layers");
#endif
}
...
//---------------------------------------------------------------------------
    ...
    if (g_fast_mode || g_stepMode || (emDisplayed >= (uint32_t)g_display_image_time)) {
        if (g_picture_loaded) {
#if defined(_ILI9341_t3NH_) || defined(_ILI9488_t3H_) || defined(_RA8876_T3) || defined(_RA8876_T41_P) || defined(__ST7735_t3_H_) || defined(__ST7789_t3_H_) || defined(__ST7796_t3_H_)
            tft.updateScreen();
...
#elif defined(__ST7735_t3_H_) || defined(__ST7789_t3_H_) || defined(__ST7796_t3_H_)
inline void FillScreen(uint16_t color) {
    tft.fillScreen(color);
}
inline uint16_t Color565(uint8_t r, uint8_t g, uint8_t b) {
...
#if defined(_RA8876_T3) || defined(_RA8876_T41_P)
//Serial.printf("WriteRect(%d, %d, %d, %d, %p)\n", x, y, cx, cy, pixels);
#if defined(_RA8876_T41_P)
    tft.useCanvas(true);
    //tft.putPicture(x, y, cx, cy,  (const unsigned char*)pixels);
...
#elif defined(_RA8876_T3) && defined(RA8876_INT) && defined(USE_FT5206_TOUCH)
void ProcessTouchScreen() {
    if (tft.touched()) {  //if touched(true) detach isr
                          //at this point we need to fill the FT5206 registers...
        tft.updateTS();   //now we have the data inside library
